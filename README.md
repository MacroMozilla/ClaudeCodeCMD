...
Claude Code 自定义命令速查。每条命令都写清楚三件事：**什么时候用**、**什么时候别用**、**有什么坑**。
第二件事是这个项目存在的理由 —— 网上的命令合集基本只写第一件。

两种用法：

**一次性用** —— 从下面展开对应命令，复制提示词，粘进 Claude Code。
...
Claude Code 自带这几个 bundled skill，**同名的自定义命令会直接覆盖它们**：
...
还有一个容易搞反的点：frontmatter 里的 **`allowed-tools` 不是限制，是免批准** —— 官方原文 *"It does not restrict which tools are available: every tool remains callable"*。
...
```
 1. 先看懂    explain-architecture
 2. 补测试    add-tests
 3. 只读审查   code-review / security-audit
 4. 挑着修    fix-issues / improve-error-handling
 5. 清场     remove-dead-code
 6. 动结构    refactor
 7. 收局部    simplify
 8. 先测量    profile
 9. 再优化    optimize
...
提示词全文在 [`commands/`](commands/) 目录里，每个文件都是可以直接用的成品（含 frontmatter）。

### ② 代码修改类

<details>
<summary><b><code>fix-issues</code></b> · 高风险 — 按已有的问题清单逐条修，一条一个 commit。</summary>
...
- 跑之前先跑 → [`code-review`](commands/code-review.md) —— 先有排好序的清单，再挑着修
- 跑之前先跑 → [`add-tests`](commands/add-tests.md) —— 要改的地方没测试就先补
- 跑完接着跑 → [`verify`](commands/verify.md) —— 每修完一条就验一次
...
<details>
<summary><b><code>remove-dead-code</code></b> · 高风险 — 删掉真正没人用的代码 —— 删之前必须先证明它是死的。</summary>
...
- 跑之前先跑 → [`code-review`](commands/code-review.md) —— 先搞清楚哪些是真残骸
- 跑之前先跑 → [`add-tests`](commands/add-tests.md) —— 删错了要有东西报警
- 跑完接着跑 → [`refactor`](commands/refactor.md) —— 清完场再动结构
- 跑完接着跑 → [`verify`](commands/verify.md) —— 删完立刻验
...
<details>
<summary><b><code>refactor</code></b> · 高风险 — 调整代码结构，对外行为一点不变。</summary>
...
- 跑之前先跑 → [`add-tests`](commands/add-tests.md) —— 必须先有行为基线
- 跑之前先跑 → [`explain-architecture`](commands/explain-architecture.md) —— 先搞清楚现在的边界在哪
- 跑之前先跑 → [`remove-dead-code`](commands/remove-dead-code.md) —— 先清场，少搬点东西
- 跑完接着跑 → [`verify`](commands/verify.md) —— 行为必须完全一致
- 跑完接着跑 → [`simplify`](commands/simplify.md) —— 结构定了再收拾局部
- 跑完接着跑 → [`profile`](commands/profile.md) —— 结构定了再测量，然后才优化
...
<details>
<summary><b><code>simplify</code></b> · 高风险 — 精简局部逻辑，不改变对外行为。</summary>
...
- 跑之前先跑 → [`add-tests`](commands/add-tests.md) —— 没有测试就别跑，改坏了发现不了
- 跑之前先跑 → [`refactor`](commands/refactor.md) —— 结构要动的话先动结构，顺序反了会白干
- 跑完接着跑 → [`verify`](commands/verify.md) —— 行为必须和跑之前一模一样，跑完立刻验
...
<details>
<summary><b><code>improve-error-handling</code></b> · 中风险 — 把吞掉的、糊弄过去的错误处理补成能排查的。</summary>
...
- 跑之前先跑 → [`code-review`](commands/code-review.md) —— 先定位哪些地方真的在吞错误
- 跑完接着跑 → [`add-tests`](commands/add-tests.md) —— 补上失败路径的测试
- 跑完接着跑 → [`verify`](commands/verify.md) —— 确认正常路径没被改坏
...
<details>
<summary><b><code>optimize</code></b> · 高风险 — 只优化 profile 指出来的地方，改完要拿数字说话。</summary>
...
- 跑之前先跑 → [`profile`](commands/profile.md) —— **必须**先测量，这是硬前置
- 跑之前先跑 → [`refactor`](commands/refactor.md) —— 结构先定下来，否则优化白做
- 跑之前先跑 → [`add-tests`](commands/add-tests.md) —— 优化最容易悄悄改坏行为
- 跑完接着跑 → [`verify`](commands/verify.md) —— 行为必须没变
- 跑完接着跑 → [`profile`](commands/profile.md) —— 改完再测一次，确认真的变快了
...
<details>
<summary><b><code>audit-dependencies</code></b> · 中风险 — 盘一遍依赖：有漏洞的、没人用的、版本落后的，先出报告再动手。</summary>
...
- 跑完接着跑 → [`verify`](commands/verify.md) —— 每升一个都要验
- 跑完接着跑 → [`security-audit`](commands/security-audit.md) —— 漏洞类的接着做深一层
...
<details>
<summary><b><code>standardize</code></b> · 低风险 — 统一格式、lint、类型和命名，纯机械改动。</summary>
...
- 跑完接着跑 → [`verify`](commands/verify.md) —— 确认纯机械改动没碰坏行为
...
<details>
<summary><b><code>add-tests</code></b> · 低风险 — 给现有行为补测试 —— 所有高风险命令的前置。</summary>
...
- 跑完接着跑 → [`refactor`](commands/refactor.md) —— 有基线了才能安全动结构
- 跑完接着跑 → [`simplify`](commands/simplify.md) —— 同上
- 跑完接着跑 → [`verify`](commands/verify.md) —— 确认新测试真的在跑
...
<details>
<summary><b><code>document</code></b> · 低风险 — 补文档和注释，写代码实际做的事，不是你希望它做的事。</summary>
...
- 跑之前先跑 → [`explain-architecture`](commands/explain-architecture.md) —— 先摸清全局再动笔
...
<details>
<summary><b><code>verify</code></b> · 只读 — 跑构建、测试、类型检查，如实报结果 —— 每次改完都跑一次。</summary>
...
- 跑完接着跑 → [`fix-issues`](commands/fix-issues.md) —— 红了的话拿着结果去修
...
<details>
<summary><b><code>code-review</code></b> · 只读 — 审查当前 diff，只出报告，一个字都不改。</summary>
...
- 跑完接着跑 → [`fix-issues`](commands/fix-issues.md) —— 拿到清单后挑着修
- 跑完接着跑 → [`add-tests`](commands/add-tests.md) —— 审出来的行为缺口先补测试再动手
...
<details>
<summary><b><code>security-audit</code></b> · 只读 — 按攻击面过一遍代码，只报不修，每条要给出可利用路径。</summary>
...
- 跑之前先跑 → [`explain-architecture`](commands/explain-architecture.md) —— 先知道信任边界在哪
- 跑完接着跑 → [`fix-issues`](commands/fix-issues.md) —— 挑确认的问题去修
- 跑完接着跑 → [`audit-dependencies`](commands/audit-dependencies.md) —— 依赖层的问题单独盘
...
<details>
<summary><b><code>explain-architecture</code></b> · 只读 — 讲清楚这个代码库实际长什么样 —— 以代码为准，不是以 README 为准。</summary>
...
- 跑完接着跑 → [`refactor`](commands/refactor.md) —— 知道边界了才好动结构
- 跑完接着跑 → [`harden`](commands/harden.md) —— 全流程改造前的第一步
- 跑完接着跑 → [`document`](commands/document.md) —— 梳理完顺手补文档
...
<details>
<summary><b><code>check-against-spec</code></b> · 只读 — 拿实现对着规格逐条核，两个方向都核。</summary>
...
- 跑完接着跑 → [`fix-issues`](commands/fix-issues.md) —— 按差距清单补齐
- 跑完接着跑 → [`add-tests`](commands/add-tests.md) —— 给规格里的每条加测试
...
<details>
<summary><b><code>profile</code></b> · 只读 — 真的跑起来测量，拿到热点数据 —— optimize 的硬性前置。</summary>
...
- 跑之前先跑 → [`refactor`](commands/refactor.md) —— 结构先定，否则测了也白测
- 跑完接着跑 → [`optimize`](commands/optimize.md) —— 拿着数据去优化，这是 optimize 的硬前置
...
<details>
<summary><b><code>harden</code></b> · 高风险 — 全流程编排：从摸架构到补文档，每个阶段停下等你确认。</summary>
...
- 跑之前先跑 → [`explain-architecture`](commands/explain-architecture.md) —— 不熟的代码库先摸清结构再启动
- 跑完接着跑 → [`pre-release`](commands/pre-release.md) —— 全流程跑完，发版前再走一遍轻量检查
...
<details>
<summary><b><code>pre-commit</code></b> · 低风险 — 提交前的轻量组合：先验一遍，再统一格式，分开提交。</summary>
...
- 跑完接着跑 → [`code-review`](commands/code-review.md) —— 改动大的话接着做完整审查
...
<details>
<summary><b><code>pre-release</code></b> · 中风险 — 发版前：安全审查 + 全量验证 + 文档对齐，每步停下。</summary>
...
- 跑之前先跑 → [`harden`](commands/harden.md) —— 要做深度整理的话先走全流程
- 跑之前先跑 → [`verify`](commands/verify.md) —— 确保出发点是绿的
...
⚠️ 内置命令的可用性随 Claude Code 版本、套餐和平台变化。**以你本地 `/help` 的输出为准** —— [官方命令文档](https://code.claude.com/docs/en/commands)。
...
改文案 = 改 `commands.json` 推一下，GitHub Pages 直接生效，没有构建步骤。
`app.js` 里不含任何具体命令内容 —— 连界面标签都在 JSON 的 `ui` 块里。

本地预览需要起个 HTTP 服务（`fetch` 读 JSON 会被同源策略挡住）：
...
内置命令、bundled skill、frontmatter 字段都对着官方文档核过（2026-08-06），不是凭记忆写的：
