...
一屏看完 19 条自定义命令，点命令名展开细节，一键复制。外加 104 条 Claude Code 内置命令速查。
...
## Claude Code 内置命令速查（104 条）

按官方命令文档整理，共 104 条（内置 90 · bundled skill 13 · workflow 1）。

> **code.claude.com 官方命令文档，快照日 2026-08-06（文档中出现的最新版本号 v2.1.222）**
>
> ⚠️ 内置命令随 Claude Code 版本、套餐和平台变化，这份不保证和你本地一致。以你本地 /help 的输出为准。

标 `skill` 的是 bundled skill（本质是一段提示词，Claude 也可能自己触发），标 `workflow` 的是多 agent 编排，其余是真正的内置命令。
...
| `/rewind` | 把对话和/或代码回滚到之前某个检查点 |
| `/background` `[prompt]` | 把当前会话转成后台 agent，腾出这个终端 |
| `/stop` | 停掉当前的后台会话 |
| `/tasks` | 查看和管理本会话的后台任务，含已完成的子 agent |
...
| `/teleport` | 把一个 Claude Code on the web 会话拉到这个终端里继续 |
| `/desktop` | 把当前会话转到 Claude Code 桌面版继续 |
| `/remote-control` | 让这个会话可以从 claude.ai 远程控制 |
...
| `/sandbox` | 开关沙箱模式 |
| `/loop` `[interval] [prompt]` `skill` | 让一个提示词按间隔反复跑；不给间隔则由 Claude 自己掌握节奏 |
| `/schedule` `[description]` | 创建/更新/列出/立即运行 Routine（跑在 Anthropic 云端的定时任务） |
...
| 命令 | 说明 |
|---|---|
| `/config` `[key=value ...]` | 打开设置界面，调主题、模型、输出风格等 |
...
| `/cost` | /usage 的别名 |
| `/stats` | /usage 的别名 |
| `/usage-credits` | 配置用量额度，或在触顶时向管理员申请 |
...
| 命令 | 说明 |
|---|---|
| `/doctor` `skill` | 跑一次安装体检，能诊断也能修：PATH、重复安装、无用 skill、慢 hook、CLAUDE.md 瘦身 |
| `/status` | 打开设置的状态页：版本、模型、账号、连通性 |
| `/help` | 显示帮助和可用命令 —— 你本地的这份才是最终依据 |
| `/bug` `[report]` | 报 bug 或分享你的对话 |
...
| `/mobile` | 显示下载 Claude 手机 App 的二维码 |
| `/exit` | 退出 CLI |
